...
# from src.data_reader import DataReader, DataReader_test, DataReader_pred, DataReader_mseed
  ...
  with tf.Session(config=sess_config) as sess:
    ...
    for epoch in range(args.epochs):
      ...
      try: ## IO Error on cluster
        flog.flush()
        pool.map(partial(ph.plot_result_thread,
                        pred = preds_batch,
                        X = X_batch,
                        Y = Y_batch,
...
def valid_fn(args, data_reader, figure_dir=None, result_dir=None):
  ...
  model = ph.Model(config, input_batch=batch, mode='valid')
  sess_config = tf.ConfigProto()
  sess_config.gpu_options.allow_growth = True
  sess_config.log_device_placement = False
  ...
  with tf.Session(config=sess_config) as sess:
    ...
    for step in progressbar:
      ...
      itp_batch = ph.clean_queue(itp_batch)
      its_batch = ph.clean_queue(its_batch)
      picks_batch = pool.map(partial(ph.postprocessing_thread,
                               pred = pred_batch,
                               X = X_batch,
                               Y = Y_batch,
                               itp = itp_batch,
    ...
    metrics_p, metrics_s = ph.calculate_metrics(picks, itp, its, tol=0.1)
...
def pred_fn(args, data_reader, figure_dir=None, result_dir=None, log_dir=None):
  ...
  model = ph.Model(config, batch, "pred")
  sess_config = tf.ConfigProto()
  sess_config.gpu_options.allow_growth = True
  sess_config.log_device_placement = False
  ...
  if args.mode == "train":
    with tf.name_scope('create_inputs'):
      ...
      if args.valid_list is not None:
        data_reader_valid = ph.DataReader(
    ...
    with tf.name_scope('create_inputs'):
      data_reader = ph.DataReader_test(
    ...
    with tf.name_scope('create_inputs'):
      if args.input_mseed:
        data_reader = ph.DataReader_mseed(
            data_dir=args.data_dir,
            data_list=args.data_list,
            queue_size=args.batch_size*10,
            coord=coord,
        ...
        data_reader = ph.DataReader_pred(
            data_dir=args.data_dir,
            data_list=args.data_list,
            queue_size=args.batch_size*10,
            coord=coord,
